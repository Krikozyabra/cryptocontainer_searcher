...
| **PGP / GnuPG** | Сигнатурный анализ (ищем только файлы зашифрованные симметричным ключом): первый байт описывает тэг 3 и последющий либо тэг 9, либо тэг 18 |
...
## Структура пакетов PGP (Симметричное шифрование)

### 1. Формирование 1-го байта заголовка PGP
Первый байт пакета читается побитово, начиная со старшего (7-го) бита:

| Биты | Назначение | Значение |
| :--- | :--- | :--- |
| **7** | **Маркер** | Всегда **`1`** (если 0 — это не PGP-пакет). |
| **6** | **Формат** | **`0`** = Старый формат, **`1`** = Новый формат. |
| **5–0** | **Тег и длина** | **В Новом:** биты 5–0 — это номер Тега (от 0 до 63).<br>**В Старом:** биты 5–2 — номер Тега (до 15), биты 1–0 — длина. |

### 2. Теги PGP при шифровании паролем
Обычно пакеты выстраиваются в цепочку: сначала **Тег 3**, за ним **Тег 18**, внутри которого зашифрованы сами данные и **Тег 19**.

| Тег | Название пакета | Суть и роль в шифровании |
| :---: | :--- | :--- |
| **3** | Symmetric-Key Session Key | **Идет первым.** Хранит параметры (соль, алгоритм, S2K) для генерации ключа из пароля пользователя. |
| **18** | Encrypted & Integrity Protected | **Современный контейнер.** Идет за Тегом 3. Содержит сами зашифрованные файлы и защищен от подделки. |
| **19** | MDC (Modification Detection Code) | **Спрятан в конце внутри Тега 18** (зашифрован). Это хеш-сумма, по которой проверяется, что файл не поврежден. |
| **9** | Symmetrically Encrypted Data | **Устаревший контейнер.** Предшественник Тега 18. Содержит данные, но *без* защиты целостности (Тега 19 внутри нет). |
...
- **GPGMEPP** — нативное дешифрование PGP/GnuPG
...
cmake -B build -G "MinGW Makefiles" -DLOG=ON
...
> `libcryptsetup`). На Windows модуль LUKS отключается на этапе
> компиляции, либо можно использовать WSL.
> 
