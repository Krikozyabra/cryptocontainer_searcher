...
#include "win_compat.h"  // easylogging pulls <windows.h>; clear COM `interface`

// The FUSE mount operations below are only needed by the encfs/encfsr binaries.
// Library consumers that just need the crypto/decrypt primitives (and cannot
// depend on libfuse, e.g. on Windows) can define ENCFS_NO_FUSE to drop the FUSE
// surface. The portability shim provides the POSIX types the core relies on.
#ifdef ENCFS_NO_FUSE
