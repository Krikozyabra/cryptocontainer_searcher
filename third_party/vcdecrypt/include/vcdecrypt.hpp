/*
 vcdecrypt — public module API.

 A small, dependency-free (for the consumer) C++ interface for reading VeraCrypt
 volumes. Include this one header and link the `vcdecrypt` static library; none
 of VeraCrypt's internal headers leak into your project.

 Cross-platform: Linux and Windows (MSVC or MinGW), C++11. Decryption only.
 ...
 -------
   ...
   opt.pim      = 0;               // Personal Iterations Multiplier (0 = default)

   try {
       vcdecrypt::Volume vol(opt);
   ...
   catch (const vcdecrypt::IncorrectPassword&) { ... }
   catch (const vcdecrypt::Error& e)           { std::cerr << e.what(); }
*/

#ifndef VCDECRYPT_PUBLIC_HPP
#define VCDECRYPT_PUBLIC_HPP
...
struct OpenOptions {
    ...
    std::vector<std::string> keyfiles;     // optional keyfiles (files only)
    bool hidden = false;                   // open the hidden volume, not the outer one
    bool useBackupHeader = false;          // use the embedded backup header
    int  pim = 0;                          // Personal Iterations Multiplier (0 = default)
};

// An opened, decryptable VeraCrypt volume. Read-only. Move-only.
class Volume {
public:
    ...
    // sectorSize(). Throws vcdecrypt::Error on failure.
